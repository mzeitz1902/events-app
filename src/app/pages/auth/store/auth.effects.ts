...
import {logIn, loginOrSignupFail, logInSuccess, logout, signUp, signUpSuccess} from "./auth.actions";
      ...
      this.actions$.pipe(
        ...
        switchMap((action) => {
          return this.authService.login$(action.user).pipe(
            ...
            catchError((err: FirebaseError) => of(loginOrSignupFail({error: err.code as AuthErrorCode})))
          );
        })
      )
  );
